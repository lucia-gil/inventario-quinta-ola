 ...
 *
 * USO:
 * <script src="/inventario/js/auth.js"></script>
 * <script>
 * Auth.requireLogin();
 * if (Auth.canApprove()) { ... }
...
const Auth = (() => {
    ...
    const ROLES = {
        'Viewer': {
            ...
            menuType: 'default',
            landing:  '/inventario/home',
        },
        'Member': {
            readable: 'Encargado de Depósito',
            menuType: 'default',
            landing:  '/inventario/deposit-view',
        },
        'Manager': {
            readable: 'Aprobador',
            menuType: 'default',
            landing:  '/inventario/dashboard',
        },
        'Administrador': {
        ...
        'SuperAdmin': {
            readable: 'SuperAdmin',
            menuType: 'superadmin',
            landing:  '/inventario/superadmin-permissions',
        },
    };
    ...
    function requireLogin() {
        if (!getUserId()) {
            window.location.href = '/inventario/AuthServlet?action=formLogin';
            return false;
        }
        return true;
    }
    ...
    function requireRole(allowedRoles) {
        ...
        if (!allowedRoles.includes(getRoleName())) {
            window.location.href = '/inventario/error404';
            return false;
        }
        return true;
    }
    ...
    function logout() {
        ...
        fetch('http://localhost:8080/inventario/api/auth/logout', {
            ...
            .catch(() => {})
            .finally(() => window.location.href = '/inventario/AuthServlet?action=formLogin');
    }
